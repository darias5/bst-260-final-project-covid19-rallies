...
To-Do List
1) Load data of Trump visits
2) Find policies (maybe state-level?)
3) Find political characteristics (2018 voting? 2016 voting?)
...
1) Re-load data and get a sense of scale of unnassigned cases and deaths per state
2) Get a sense of the difference between counties trump visited and the average county. 
...
states <- as_tibble(map_data('state')) %>%
  rename(state = region) %>%
  left_join(df_snapshot, by = "state") %>%
  dplyr::select(-subregion)

## Map 1: Population 

map_population <- ggplot() + 
  geom_polygon(data = states, aes(x= long, y = lat, fill = population, group = group)) +
...
map_gov <- ggplot() + 
  geom_polygon(data = states, aes(x= long, y = lat, fill = gov_control, group = group)) +
  theme_nothing(legend = TRUE) + coord_map() + 
  ggtitle("Governor's Political Party in 2020") + 
  theme(plot.title = element_text(hjust =0.5))+
  scale_fill_gradient(low = "blue", high = "red", guide = "legend", breaks= c(0,1), labels = c("Democrat", "Republican"), name = "Republican / Democrat")

map_sip <- ggplot() + 
  geom_polygon(data = states, aes(x= long, y = lat, fill = days_since_enactment, group = group)) +
  theme_nothing(legend = TRUE) + coord_map() + 
  geom_polygon(color = "black") +
  ggtitle("Days since Shelter-In-Place Enacted as of April 26th, 2020") + 
  theme(plot.title = element_text(hjust =0.5)) +
  scale_fill_distiller(palette ="Greys", direction = 1, ) # or direction=1
...
ggsave(filename = file.path(outputpath,"map_gov.pdf"), map_gov)
ggsave(filename = file.path(outputpath,"map_sip.pdf"), map_sip)

rm(map_population, map_fatalities, map_cases, map_gov, map_sip)
