...
# Creating a variable that contains the number of deaths
  df <- df %>% arrange(date) %>% 
  group_by(fips)  %>% 
...
library(readxl)
rally_data <- read_excel("bst-260-final-project-covid19-rallies/data/rallies_cc_da.xlsx")
...
df <- df %>%
  ...
  mutate(rally_ind = ifelse(is.na(rally), 0, 1))
df$rally <- NULL

temp <- df %>% 
  group_by(fips) %>% 
  filter(sum(rally_ind) != 0) %>%
  mutate(day_to_rally = date - date[which(rally_ind == 1)]) %>%
  select(date, fips, day_to_rally)
...
```
```{r}
land_area <- read_excel(file = file.path(datapath,"Land area by counties.xlsx"))


land_area$fips <- as.numeric(land_area$fips)


#Merging land area in Km and creating density variable
df <- df %>% left_join(land_area[,c(2,3)],by="fips") %>% mutate(density = population/land_area)
...
fit <- glm(rally_ind ~ median_inc + density ,data=df)

summary(fit)

df$score <- predict(fit, df)


# we do not have median income for some counties that had rallies
```
