...
df_singledata <- df %>% filter(date == "2020-08-01") 
rally_singledata <- rally %>% filter(date <= "2020-08-01") 
...
df_singledata %>%
  ...
  geom_polygon() + 
    ...
    scale_color_continuous(low = "white", high = "red",
                               limits = c(0,150),    # Forcing upper limit of 150 cases per day
                               oob = scales::squish)+ # Letting values out of bounds be squished to scale
  geom_point(data=rally_singledata, aes(x=long, y=lat), color="red", alpha=0.5)
```
...
df %>%
  ...
  distinct() %>%
  filter(rally_ind == 1) %>%  
  filter(abs(day_to_rally_1) <= 21) %>%
  ggplot(aes(x = day_to_rally_1, y = new_deaths_7dayavg_per_mil_cap, color = as.factor(fips))) +
  geom_line() + 
  theme_bw()
